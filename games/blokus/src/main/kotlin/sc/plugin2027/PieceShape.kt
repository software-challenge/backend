...
enum class PieceShape(coordinates: Set<Coordinates>) {
    ...
    val asVectors: Set<Vector> by lazy {
        coordinates.mapTo(HashSet()) {it - Coordinates(0, 0)}
    }
    ...
    init {
        val mapVariants = HashMap<Set<Coordinates>, Pair<Rotation, Boolean>>()
        val mapTransformations = HashMap<Pair<Rotation, Boolean>, Set<Coordinates>>()
    ...
    fun transform(rotation: Rotation, shouldFlip: Boolean): Set<Coordinates> =
        transformations[rotation to shouldFlip] ?: HashSet()
    
    companion object {
        /** Eine Map, die anhand des Index' des Enums die entsprechende Form zurückgibt. */
        val shapes: Map<Int, PieceShape> = ((0 until Constants.TOTAL_PIECE_SHAPES) zip PieceShape.entries).toMap()
    }
}