        ...
        "constructed" should {
            ...
            "have each PieceShape available for each color" {
                Color.entries.forEach { color ->
                    state.undeployedPieceShapes(color) shouldBe PieceShape.entries.toSet()
                }
            }
            "start with no points for either player" {
                state.getPointsForTeam(Team.ONE)[0] shouldBe 0
                state.getPointsForTeam(Team.TWO)[0] shouldBe 0
            }
        }
        "turn number increases" should {
            "advance turn, round and currentcolor accordingly" {
                GameState().run {
                    ...
                    Color.entries.filterNot { it == Color.RED }.forEach { color ->
...
//                    round shouldBe 6
        ...
        "a piece is placed a second time" should {
            ...
            "throw an InvalidMoveException" {
                ...
                ex.mistake shouldBe BlokusMoveMistake.DUPLICATE_SHAPE
                state.undeployedPieceShapes(Color.BLUE).size shouldBe 20
            }
            "allow a SkipMove" {
        ...
        "serialised and deserialised" should {
            ...
            "equal the original GameState" {
                ...
                transformed.getPointsForTeam(Team.ONE)[0] shouldBe 0
