...
echo "IP: $1" >> $log
echo "VMName: $2" >> $log
SSH_OPTIONS="-q -o StrictHostKeyChecking=no -o BatchMode=true -o ConnectTimeout=5 -o UserKnownHostsFile=/dev/null -i $HOME/.ssh/client_key"
ssh -l scadmin $SSH_OPTIONS $1 "head -c ${MAX_FILE_SIZE_BYTES} ~/logs/exec.log" > ~/clientlogs/$2.log
