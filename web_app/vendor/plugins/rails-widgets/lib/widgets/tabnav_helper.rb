  ...
  module TabnavHelper
    ...
    def tabnav name, opts={}, &block
      ...
      if block_given?
        ...
        html << tag('div', options, true).html_safe!
        html << capture(&block)
        html << '</div>'.html_safe!
    ...
    def render_tabnav(name, opts={}, &proc)
      ...
      concat tag('div',@_tabnav.html ,true).html_safe!
    ...
    def render_tabnav_tabs
      ...
      @_tabnav.tabs.each do |tab|
        ...
        concat tag('li', li_options, true).html_safe!
        if tab.disabled? || (tab.link.empty? && tab.remote_link.nil?)
          concat content_tag('span', tab.name, tab_html).html_safe!
        elsif !tab.link.empty?
          concat link_to(tab.name, tab.link, tab_html).html_safe!
          ...
          concat link_to_remote(tab.name, remote_opts.merge(tab.remote_link), tab_html).html_safe!
        else
          raise "WHAT THE HELL?"
        end
        concat "</li>\n".html_safe!
