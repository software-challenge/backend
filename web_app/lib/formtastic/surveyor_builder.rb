  ...
  class SurveyorBuilder < SemanticFormBuilder
    ...
    def surveyor_check_boxes_input(method, options)
      ...
      list_item_content = collection.map do |c|
        ...
        li_content << basic_input_helper(:text_field, :string, :string_value, options).html_safe if options[:response_class] == "other_and_string"
        li_content << basic_input_helper(:text_field, :string, :string_value, options).html_safe if %w(string other_and_string).include?(options[:response_class])
    ...
    def surveyor_radio_input(method, options)
      ...
      list_item_content = collection.map do |c|
        ...
        li_content << basic_input_helper(:text_field, :string, :integer_value, options).html_safe if options[:response_class] == 'integer'
        li_content << basic_input_helper(:text_field, :string, :string_value, options).html_safe if options[:response_class] == 'string'
