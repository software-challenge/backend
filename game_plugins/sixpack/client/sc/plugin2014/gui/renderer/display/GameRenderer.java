...
public class GameRenderer extends JComponent {
	...
	public void updateGameState(GameState gameState) {
		...
		currentPlayer = this.gameState.getCurrentPlayer().getPlayerColor();
		updateBuffer = true;
		...
		redStones.clear();
		for (int i = 0; i < this.gameState.getRedPlayer().getStones().size(); i++) {
			Stone stone = this.gameState.getRedPlayer().getStones().get(i);
			if (stone != null) {
				redStones.add(new GUIStone(stone, i));
			}
		}

		blueStones.clear();
		for (int i = 0; i < this.gameState.getBluePlayer().getStones().size(); i++) {
			Stone stone = this.gameState.getBluePlayer().getStones().get(i);
			if (stone != null) {
				blueStones.add(new GUIStone(stone, i));
			}
		}
		...
		if (this.gameState.gameEnded()) {
			// gameEnded = true;
			currentPlayer = this.gameState.winner();
		}
