...
public class Slot implements ISlot {
    ...
    private final GuiClient adminclient;
    ...
    public void asHuman() throws IOException {
        ...
        if (!RenderFacade.getInstance().getAlreadyCreatedPlayerOne()) {
            humanClient = new GuiClient(adminclient.getHost(),
                    adminclient.getPort(), EPlayerId.PLAYER_ONE);
        }
        else {
            humanClient = new GuiClient(adminclient.getHost(),
                    adminclient.getPort(), EPlayerId.PLAYER_TWO);
        }
