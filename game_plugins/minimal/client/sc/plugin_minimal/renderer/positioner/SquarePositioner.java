...
public class SquarePositioner implements Positioner{
	...
	public SquarePositioner(Node node) {
		this.node=node;		
	...
	public void scale(int size) {
		this.size = size;
		r = 1.0/2.0*Math.sqrt(Math.pow(node.getCenterX()-node.getXs()[0],2)
	...
	private void createScaledPositions(int n){
		...
		for(int i=0;i<n;i++){
			this.xs[i] = node.getScaledCenterX() +(int) ((((n>1)?(size*r*Math.cos(2.0*i*Math.PI/n)):0)));
			this.ys[i] = node.getScaledCenterY() +(int) ((((n>1)?(size*r*Math.sin(2.0*i*Math.PI/n)):0)));
		}
	}
