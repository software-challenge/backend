...
public class RenderFacade {
  private static final Logger logger = LoggerFactory.getLogger(RenderFacade.class);
  ...
  public void gameEnded(GameResult data, EPlayerId target, PlayerColor color, String errorMessage) {
    ...
    if (data == null) {
      ...
      // if one player has a non-regular cause, this is likely the cause for the
      // game end
      int i = 0;
      ...
      if (reason.length() == 0) {
        if (errorMessage != null) {
          ...
          // looks like a regular end, display score reason (both players should
          // have same reason)
