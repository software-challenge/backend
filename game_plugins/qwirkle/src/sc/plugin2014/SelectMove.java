...
import sc.plugin2014.util.SelectMoveConverter;
...
public class SelectMove extends Move implements Cloneable {
     ...
     * 
     * @param selections
     *            die jeweilige Anzahl Bausteine
     *            der Groesse i+1
     */
        ...
        for (int i = 0; i < selections.length; i++) {
            if (selections[i] < 0) {
                throw new IllegalArgumentException(
                        "Auswahlzug konnte nicht konstruiert werden: Negative Anzahl eines Bauelementes gewählt!");
            }
    ...
    void perform(GameState state, Player player) throws InvalideMoveException {
        ...
        for (int i = 0; i < MAX_SEGMENT_SIZE; i++) {
            if (selections[i] < 0) {
                throw new InvalideMoveException(
                        "Illegale Auswahl: Negative Anzahl eines Bauelementes gewählt");
            }
        ...
        if (selectionSum != SELECTION_SIZE) {
            throw new InvalideMoveException(player.getDisplayName()
                    + " hat nicht genau " + SELECTION_SIZE
                    + " Bauelemente gewählt");
        }
