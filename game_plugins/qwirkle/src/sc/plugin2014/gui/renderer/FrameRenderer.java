...
import static sc.plugin2014.gui.renderer.RenderConfiguration.*;
...
public class FrameRenderer extends JComponent {
    ...
    private static final int         TOWER_TOTAL_WIDTH   = TOWER_LEFT_WIDTH
                                                                 + TOWER_RIGHT_WIDTH;
    ...
    private static final int         CARD_WIDTH          = ((8 + (SLOTS * 6)) - 6)
                                                                 + CARD_DOT;
    private static final int         CARD_HEGTH          = (8 + (SLOTS * (CARD_DOT + 4))) - 4;

    private static final int         MAX_SEGMENT_HEIGTH  = (MAX_SEGMENT_SIZE * TOWER_STORIE_HEIGTH)
                                                                 + TOWER_RIGHT_HEIGTH
                                                                 + TOWER_LEFT_HEIGTH;

    // schrift
    private static final Font        h0                  = new Font(
                                                                 "Helvetica",
                                                                 Font.BOLD, 73);
    private static final Font        h1                  = new Font(
                                                                 "Helvetica",
                                                                 Font.BOLD, 42);
    private static final Font        h2                  = new Font(
                                                                 "Helvetica",
                                                                 Font.BOLD, 27);
    private static final Font        h3                  = new Font(
                                                                 "Helvetica",
                                                                 Font.BOLD, 23);
    private static final Font        h4                  = new Font(
                                                                 "Helvetica",
                                                                 Font.BOLD, 14);
    private static final Font        h5                  = new Font(
                                                                 "Helvetica",
                                                                 Font.PLAIN, 13);

    private static final JPanel      fmPanel             = new JPanel();
    private static final FontMetrics fmH0                = fmPanel
                                                                 .getFontMetrics(h0);
    // private static final FontMetrics fmH1 = fmPanel.getFontMetrics(h1);
    private static final FontMetrics fmH2                = fmPanel
                                                                 .getFontMetrics(h2);
    private static final FontMetrics fmH3                = fmPanel
                                                                 .getFontMetrics(h3);
    private static final FontMetrics fmH4                = fmPanel
                                                                 .getFontMetrics(h4);
    private static final FontMetrics fmH5                = fmPanel
                                                                 .getFontMetrics(h5);
    ...
    private static final String      SELECT_STRING       = Constants.SELECTION_SIZE
                                                                 + " Bauelemente auswählen";
    private static final int         MIN_DIALOG_SIZE     = fmH2.stringWidth(SELECT_STRING)
                                                                 + (4 * GAP_SIZE);
    private static final int         STATUS_HEIGTH       = PROGRESS_BAR_HEIGTH
                                                                 + (2 * STUFF_GAP)
                                                                 + Math.max(
                                                                         CARD_HEGTH,
                                                                         MAX_SEGMENT_HEIGTH)
    ...
    private static final int[]       js                  = new int[] { 0, 0, 0,
            2, 1, 0                                     };
    ...
    private final MouseAdapter       selectMouseAdapter  = new MouseAdapter() {
                                                             ...
                                                             public void mousePressed(
                                                                 ...
                                                                 if (selectionOkay) {
                                                                     ...
                                                                     if ((x > selectButton[0])
                                                                             && (x < (selectButton[0] + selectButton[2]))) {
                                                                         if ((y > selectButton[1])
                                                                                 && (y < (selectButton[1] + selectButton[3]))) {
                                                             ...
                                                             public void mouseDragged(
                                                                 ...
                                                                 if (selectedSegment != null) {
                                                                     ...
                                                                     for (TowerData tower : sensetiveTowers) {
                                                                         if (inner(
                                                                                 selectedSegment.xs[1],
                                                                                 selectedSegment.ys[1],
                                                                                 tower.xs,
                                                                                 tower.ys)) {
                                                                             if (tower.slot > 0) {
    ...
    public FrameRenderer() {
        ...
        setMinimumSize(new Dimension(
                (2 * CARDS_PER_PLAYER * (CARD_WIDTH + STUFF_GAP))
                        + (2 * SELECTION_SIZE * (TOWER_TOTAL_WIDTH + STUFF_GAP)),
                600));
    ...
    public void updateGameState(GameState gameState) {
        ...
        currentMoveType = gameState.getCurrentMoveType();
        ...
        highestSegments[0] = gameState.getRedPlayer()
                .getHighestCurrentSegment();
        highestSegments[1] = gameState.getRedPlayer().getHighestSegment();
        highestSegments[2] = gameState.getBluePlayer()
                .getHighestCurrentSegment();
    ...
    private synchronized void moveSegment(final GameState gameState) {
        ...
        if (OPTIONS[MOVEMENT]) {
            ...
            int h = (selectedSegment.size * TOWER_STORIE_HEIGTH)
            ...
            for (int frame = 0; frame < frames; frame++) {
                ...
                repaint(selectedSegment.xs[0] - 5, selectedSegment.ys[4] - 5,
    ...
    private void recreateCityTowers() {
        ...
        int cityWidth = (TOWER_LEFT_WIDTH + (SLOTS * (TOWER_RIGHT_WIDTH + DIAGONAL_GAP)))
                - DIAGONAL_GAP;
    ...
    private void recreateSelectDialog() {
        ...
        synchronized (selectTowers) {
            ...
            selectHeight += differentSegments
                    * (GAP_TOWER_TOP + GAP_TOWER_BOTTOM + (2 * GAP_SIZE));
            selectHeight += differentSegmentsAccN * TOWER_STORIE_HEIGTH;
            selectWidth = Math.max(MIN_DIALOG_SIZE, mostSegments
                    * (DIST_TOWER_SELECT + TOWER_TOTAL_WIDTH));
            ...
            for (Segment segment : segments) {
                int retained = segment.getRetained();
                if (retained > 0) {
                    y += (2 * GAP_SIZE) + GAP_TOWER_TOP + GAP_TOWER_BOTTOM
                            + (segment.size * TOWER_STORIE_HEIGTH);
            ...
            selectButton = new int[] { (getWidth() - SIDE_BAR_WIDTH - 200) / 2,
                    (selectY + selectHeight) - GAP_SIZE - fmH2.getHeight(),
                    200, fmH2.getHeight() };
    ...
    private void recreatePlayerSegments() {
        ...
        int x = BORDER_SIZE + STUFF_GAP
                + (CARDS_PER_PLAYER * (CARD_WIDTH + STUFF_GAP));
    ...
    private void paintTower(Graphics2D g2, TowerData tower, boolean cityTower,
            boolean glow) {
    ...
    private void paintCard(Graphics2D g2, int x, int y, int slot) {
        ...
        g2.fillRect(x + 4 + (slot * 6), (y + CARD_HEGTH) - 12 - (slot * 12), 8,
                8);
