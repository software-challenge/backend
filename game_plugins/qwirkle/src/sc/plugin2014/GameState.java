...
public class GameState implements Cloneable {
    ...
    public Object clone() throws CloneNotSupportedException {
        ...
        if (cardStack != null) {
            clone.cardStack = new LinkedList<Card>(cardStack);
        }
        if (usedStack != null) {
            clone.usedStack = new LinkedList<Card>(usedStack);
        }
        if (towers != null) {
            clone.towers = new LinkedList<Tower>(towers);
        }
        return clone;
    }
                    ...
                    if (tower.slot == card.slot) {
                        for (Segment segment : segments) {
                            if (tower.canAddPart(player, segment.size)) {
                                moves.add(new BuildMove(tower.city, tower.slot,
                                        segment.size));
                            }
                        }
                    }
