...
darf auf dem Feld des Gegners oder einer Sandbank enden. Es ist weiterhin nicht
erlaubt, durch das gesamte letzte aufgedeckte Segment in ein noch nicht
...
bearbeiten kann. Zuerst kann man den Namen des ersten und zweiten Spielers festlegen, indem man in das jeweilige Namensfeld klickt. Daneben befindet sich die Auswahl, ob der jeweilige Spieler ein Mensch, ein Computerspieler oder ein manuell gestarteter Computerspieler sein soll. Wählt man hier ``Computerspieler'' erscheint ein Dateiauswahldialog, in dem man das JAR-Archiv mit dem Computerspieler auswählen kann. In das letzte Feld können noch Parameter eingegeben werden, die dem Computerspieler beim Start übergeben werden sollen (dies ist nur bei Spielertyp ``Computerspieler'' relevant).
...
\emph{Nach dem Starten eines neuen Spiels muss immer zuerst der ``Pause''-Knopf unten links angeklickt werden, sonst geht das Spiel nicht voran.}

\subsection{Spielersteuerung}

Wenn ein menschlicher Spieler am Zug ist, kann er die Aktionen, die er in seinem
Zug durchführen will, über die Spielersteuerung eingeben. Die Spielersteuerung
...
Durch Klicken auf ein Spielersteuerungselement wird die damit verbundene Aktion
...
einen Anleger fahren. Beendet man dort seinen Zug, wird der Passagier automatisch
aufgenommen.

\subsection{Spielbedienung}

Die Spielbedienung am unteren Rand des Fensters steuert das Abspielverhalten des aktuellen Spiels. Diese können verwendet werden, um sich bereits getätigte Züge noch einmal anzusehen. Die einzelnen Elemente sind von links nach rechts:

\begin{description}
\item [Pause / Abspielen Knopf] Steht beim Start des Spiels auf ``Pause''. Damit das Spiel ablaufen kann, muss er auf ``Abspielen'' stehen.
\item [Geschwindigkeit-Schieberegler] Bestimmt die Zeit, die ein Zug beim Abspielen angezeigt wird.
\item [Zurück an den Anfang Knopf] Setzt das Spiel an den Anfang zurück.
\item [Schritt zurück Knopf] Geht eine Runde zurück.
\item [Schritt vor Knopf] Geht eine Runde vor.
\item [Vorspulen Knopf] Geht ans Ende des bisher gespielten Spieles.
\item [Spiel abbrechen Knopf] Beendet das Spiel.
\end{description}
