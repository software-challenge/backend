...
/**
 * Ein Kara Frontend welche den aktuellen Spielstatus darstellt und eine
 * einfache Handhabung in deutscher Sprache erlaubt.
 * 
 * @author and
 */
public class Spielstatus {
	...
	/**
	 * Erzeugt eine Instanz des Kara Frontends für das übergebene GameState
	 * Objekt
	 * 
	 * @param state
	...
	public Spielstatus(GameState state) {
		...
		for (Sheep s : state.getSheeps()) {
			...
			if (s.owner == own.getPlayer().getPlayerColor()) {
				besitzer = own;
			} else if (s.owner == opp.getPlayer().getPlayerColor()) {
				besitzer = opp;
			}
			schafe.add(new Schaf(s, besitzer, s.getNode(), this));
		}
		blumen = new LinkedList<Blume>();
		for (Flower f : state.getAllFlowers())
			blumen.add(new Blume(f, felder.get(f.node)));
	...
	public List<Schaf> holeSchafeAufFeld(int position) {
		LinkedList<Schaf> sheeps = new LinkedList<Schaf>();
		for (Schaf schaf : schafe) {
			if (schaf.holePosition().holeIndex() == position)
				sheeps.add(schaf);
		}
		return sheeps;
	}
	...
	/**
	 * Liefert die Blume(n) auf dem Feld mit dem gegebenen Index gibt es dort
	 * keine Blume, so wird null zurückgegeben
	 */
	public Blume holeBlumeAufFeld(int index) {
		for (Blume blume : blumen) {
			if (blume.holeFeld().holeIndex() == index)
				return blume;
		}
		return null;
	}
	...
	/**
	 * Liefert verfügbare Wuerfel als int Array a[0] = 0 => 0 Wuerfel mit Wert 0
	 * verfügbar ... a[5] = 2 => 2 Wuerfel mit Wert 5 verfügbar
	 * 
	...
	/**
	 * Gibt den eigenen Status zurueck [0] hat gewonnen? [1] Schafe im Spiel [2]
	 * gefangene Schafe [3] gestohlene Schafe [4] gesammelte Blumen [5]
	 * gefressene Blumen [6] Punkte
	 */
	...
	/**
	 * Gibt den gegnerischen Status zurueck [0] hat gewonnen? [1] Schafe im
	 * Spiel [2] gefangene Schafe [3] gestohlene Schafe [4] gesammelte Blumen
	 * [5] gefressene Blumen [6] Punkte
	 */
	...
	/**
	 * Gibt zurueck ob der Zug des Schafes mit dem angegeben Index auf das Feld
	 * mit dem gegebenen Index moeglich ist
	 */
	public boolean istZugMoeglich(int schafIndex, int zielIndex) {
		return istZugMoeglich(holeSchafMitIndex(schafIndex),
				holeFeldMitIndex(zielIndex));
	}
	...
	/**
	 * Gibt zurueck ob der Zug des gegebenen Schafes auf das Feld mit dem
	 * gegebenen Index moeglich ist
	 */
	...
	/**
	 * Gibt zurueck ob der Zug des gegebenen Schafes auf das gegebene Feld
	 * moeglich ist
	 */
	...
	/**
	 * Gibt die von dem gegebenen Schaf mit den aktuell verfuegbaren Wuerfeln
	 * erreichbaren Felder zurueck
	 */
	...
	public List<Zug> holeMoeglicheZuegeFuerSpieler(Spieler spieler) {
		List<Move> moves = state.getValidMoves(spieler.getPlayer()
				.getPlayerColor());
